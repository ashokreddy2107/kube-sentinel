  ...
  const podColumns = useMemo(
    (): Column<PodWithMetrics>[] => [
      ...
      },
      {
        header: 'Ready',
        accessor: (pod: Pod) => {
          // getPodStatus is computationally expensive, but memoized internally via WeakMap.
          // By calling it here (in accessor), we only compute it for visible rows (pagination).
